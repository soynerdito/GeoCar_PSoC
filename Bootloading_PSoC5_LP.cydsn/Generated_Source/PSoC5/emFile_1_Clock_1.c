...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_Start
********************************************************************************
*
* Summary:
...
void emFile_1_Clock_1_Start(void) 
{
    ...
    emFile_1_Clock_1_CLKEN |= emFile_1_Clock_1_CLKEN_MASK;
	emFile_1_Clock_1_CLKSTBY |= emFile_1_Clock_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_Stop
********************************************************************************
*
* Summary:
...
void emFile_1_Clock_1_Stop(void) 
{
    ...
    emFile_1_Clock_1_CLKEN &= (uint8)(~emFile_1_Clock_1_CLKEN_MASK);
	emFile_1_Clock_1_CLKSTBY &= (uint8)(~emFile_1_Clock_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((emFile_1_Clock_1_CLKEN & emFile_1_Clock_1_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = emFile_1_Clock_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = emFile_1_Clock_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* emFile_1_Clock_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(emFile_1_Clock_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(emFile_1_Clock_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = emFile_1_Clock_1_GetSourceRegister();
    uint16 oldDivider = emFile_1_Clock_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = emFile_1_Clock_1_CLKEN & emFile_1_Clock_1_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(emFile_1_Clock_1_DIV_PTR, clkDivider);
                emFile_1_Clock_1_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                emFile_1_Clock_1_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(emFile_1_Clock_1_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((emFile_1_Clock_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(emFile_1_Clock_1_DIV_PTR, clkDivider);
				emFile_1_Clock_1_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(emFile_1_Clock_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    emFile_1_Clock_1_MOD_SRC |= modeBitMask & (uint8)emFile_1_Clock_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    emFile_1_Clock_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(emFile_1_Clock_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return emFile_1_Clock_1_MOD_SRC & (uint8)(emFile_1_Clock_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = emFile_1_Clock_1_GetDividerRegister();
    uint8 oldSrc = emFile_1_Clock_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        emFile_1_Clock_1_MOD_SRC |= CYCLK_SSS;
        emFile_1_Clock_1_MOD_SRC =
            (emFile_1_Clock_1_MOD_SRC & (uint8)(~emFile_1_Clock_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return emFile_1_Clock_1_MOD_SRC & emFile_1_Clock_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: emFile_1_Clock_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    emFile_1_Clock_1_PHASE = clkPhase & emFile_1_Clock_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: emFile_1_Clock_1_GetPhase
********************************************************************************
*
* Summary:
...
{
    return emFile_1_Clock_1_PHASE & emFile_1_Clock_1_PHASE_MASK;
}
